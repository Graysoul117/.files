...
do
    ...
    awesome.connect_signal("debug::error", function (err)
        ...
        naughty.notify({ preset = naughty.config.presets.critical,
                         title = " Its RAINBOW FUCKING RANDOLPH!!!!",
                         text = tostring(err) })
        in_error = false
    end)
end
...
local browser2          = "qutebrower"
local browser3          = "firefox"
local editor            = os.getenv("EDITOR") or "nano"
local editorgui         = "sublime"
...
globalkeys = my_table.join(
    ...
    awful.key({ modkey, altkey   }, "Return", function() awful.util.spawn( "alacritty" ) end,
        {description = terminal, group = "alt+ctrl"}),
    ...
    awful.key({ altkey, "Control" }, "a", function () awful.screen.focus_relative( 1) end,
              {description = "focus the next screen", group = "screen"}),
    awful.key({ altkey, "Control" }, "d", function () awful.screen.focus_relative(-1) end,
...
--awful.key({ modkey, "Shift"   }, "a",   function (c) c:move_to_screen()               end,
              --{description = "move to screen", group = "client"}),

--awful.key({ modkey, "Shift"   }, "s",  function (c) c:move_to_screen()               end,
              --{description = "move to screen", group = "client"}),    

  awful.key({ modkey, "Shift"   }, "m", function() awful.util.spawn( "magnus" ) end,
        {description = magnus, group = "client"}),
 ...
 --CLG End-- 
   ...
   -- awful.key({ modkey }, "Right", function () lain.util.tag_view_nonempty(1) end,
    ...
    -- Layout manipulation
    awful.key({ altkey, "Shift"   }, "Left", function () awful.client.swap.byidx(  1)    end,
              {description = "swap with next client by index", group = "client"}),
    awful.key({ altkey, "Shift"   }, "Right", function () awful.client.swap.byidx( -1)    end,
 ...
 -- Show/Hide Systray
    ...
    -- Dynamic tagging
    awful.key({ modkey, "Shift" }, "t", function () lain.util.add_tag() end,
...
awful.rules.rules = {
    ...
    --{ rule = { class = "Thunar" },
      --properties = { screen = 3, tag = awful.util.tagnames[1],switchtotag = true  } },
...
--          properties = { callback = function (c) c.maximized = false end } },
    ...
    { rule = { name = "Create New Folder" },            properties = { floating = true } },

    { rule = { name = "Empty Trash" },                  properties = { floating = true } },    
